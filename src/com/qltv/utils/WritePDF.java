...
public class WritePDF {
	...
	public void xuatPDFPhieuNhap(JTable table, int ma) {
		...
		try {
			...
			// xuống dòng
			PhieuNhap pn = PhieuNhapDAO.getphieunhap(ma);
			System.out.println(pn);
			Chunk glue = new Chunk(new VerticalPositionMark());// Khoang trong giua hang

			Paragraph paraMaHoaDon = new Paragraph(
			new Phrase("Phiếu Nhập: " + String.valueOf(pn.getMa()), fontData));
			doc.add(paraMaHoaDon);
			doc.add(Chunk.NEWLINE);

			NhaCungCap ncc = NhaCungCapDAO.getNhacungcap(pn.getMancc());
			// viết 1 hàm getnhacungcap để lấy tên nhà cung cấp
			NhanVien nv = NhanVienDAO.getnhanvien(pn.getManv());
			...
			para2.add(String.valueOf("Tên Nhân Viên : " + nv.getTen()));
			para2.add(glue);
			doc.add(para2);
			doc.add(Chunk.NEWLINE);
